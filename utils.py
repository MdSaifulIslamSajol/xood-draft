...
def get_torch_device():
    print("utils.py ==> get_torch_device()")
    return torch.device("cuda" if torch.cuda.is_available() else "cpu")  # cuda:5


def df_to_pdf(df: pd.DataFrame, decimals: int, path, percent=True, vmin=-1, vmax=1):
    print("utils.py ==> df_to_pdf()")
    plt.clf()
    cmap = LinearSegmentedColormap.from_list('rg', ["r", "w", "g"], N=256)
    plt.figure(figsize=(((decimals + 2) * len(df.columns)) / 5, len(df.index) / 3))
    im = plt.matshow(df, alpha=1, aspect="auto", fignum=0,
                     vmin=vmin, vmax=vmax, cmap=cmap)
    font = {'weight': 'bold',
            'size': 12}
    ax = plt.gca()
    for (i, j), x in np.ndenumerate(df.to_numpy()):
        x = str(round(100 * x, decimals - 2)
                )[:decimals + 1] if percent else str(round(x, decimals))[:decimals + 2]
...
def binary_class_hist(feature: pd.Series, label: pd.Series, ax, name, label_0="dirty", label_1="clean", bins=100,
                      x_range=None):
    print("utils.py ==> binary_class_hist()")
    auc = roc_auc_score(label, feature)
    if ax:
        if x_range is None:
            x_range = feature.min(), feature.max()
        ax.hist(feature[~ label], bins=bins, color="r",
                label=label_0, alpha=0.5, density=True, range=x_range)
        ax.hist(feature[label], bins=bins, color="g",
                label=label_1, alpha=0.5, density=True, range=x_range)
        ax.set_xlabel(name)
...
def plot_model_performance(history, save_plt_path):
    print("utils.py ==> plot_model_performance()")
...
def corr_feat_selection(df: pd.DataFrame, thresh=.95, method="pearson"):
    print("utils.py ==> corr_feat_selection()")
